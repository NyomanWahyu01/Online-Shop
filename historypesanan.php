          ...
          <div class="container">
            ...
            <div class="row border">
              <div class="col-xs-12 col-sm-2">
                <strong class="title">ID Pesanan</strong>
              </div>
              <div class="col-xs-12 col-sm-4">
                <strong class="title">Tgl Pesanan</strong>
              </div>
              <div class="col-xs-12 col-sm-2">
            ...
            $query = mysqli_query(
              $koneksi,
              "select * from data_pesanan where id_user = $id_user"
            );
            while ($data = mysqli_fetch_array($query)) {
              echo "
              <div class='row border'>
              <div class='col-xs-12 col-sm-2'>
                <strong class='price'>$data[id_pesanan]</strong>
              </div>
              <div class='col-xs-12 col-sm-4'>
                <strong class='price'>$data[tgl_pesanan]</strong>
              </div>
              <div class='col-xs-12 col-sm-2'>
                <strong class='price'>" . format_rupiah($data['total_pesanan']) . "</strong>
              </div>
              <div class='col-xs-12 col-sm-2'>
                <strong class='price'>$data[status]</strong>
              </div>
              <div class='col-xs-12 col-sm-2'>
                  <form action='invoice.php' method='get' class='coupon-form'>
                      <fieldset>
                          <input type='hidden' name='id_pesanan' value='$data[id_pesanan]'>
                          <button type='submit' style='margin-top: 15px;'>Detail</button>
                      </fieldset>
                  </form>
              </div>
